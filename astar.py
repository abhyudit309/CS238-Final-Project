...
class AStar(object):
    ...
    def __init__(self, statespace_lo, statespace_hi, x_init, x_goal, occupancy, resolution=1)->None:
        ...
        self.est_cost_through = np.inf*np.ones(((self.statespace_hi - self.statespace_lo)//self.resolution + 1).astype(int))  # 2d map of the estimated cost from start to goal passing through state (often called f score)
        self.cost_to_arrive = np.inf*np.ones(((self.statespace_hi - self.statespace_lo)//self.resolution + 1).astype(int))    # 2d map of the cost-to-arrive at state from start (often called g score)
    ...
    def is_free(self, x):
        ...
        if (x_array >= self.statespace_lo).all() and (x_array <= self.statespace_hi).all():
            inside_map = True
        if inside_map and self.occupancy.is_free(x):
            return True
        else:
            return False
    ...
    def snap_to_grid(self, x):
        ...
        """
        return (int(self.resolution*round(x[0]/self.resolution)), int(self.resolution*round(x[1]/self.resolution)))
...
class DetOccupancyGrid2D(object):
    ...
    def plot(self, fig_num=0, color='blue'):
